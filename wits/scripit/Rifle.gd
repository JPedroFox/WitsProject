...
var weapon_name = 3  # Nome da arma atual
...
func _physics_process(_delta):
	...
	if Input.is_action_pressed("shoot") and animation_player.current_animation != "shot" and not is_reloading and Global.start:
		shoot()
	
	var ammo_data = storage_bullet.get_ammo(weapon_name)
	
	# Se a munição acabar e a arma não estiver recarregando, inicia a recarga
	if Input.is_action_just_pressed("shoot") and ammo_data["current_ammo"] == 0 and ammo_data["storage_ammo"] > 0 and not is_reloading:
		start_reload()
...
func shoot():
	...
	if ammo_data["current_ammo"] > 0:
		...
		if player and player.has_node("ShootPoint"):
			...
			new_bullet.direction = Vector2.RIGHT.rotated(shoot_point.global_rotation)  # Direção ajustada
